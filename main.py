...
def handle_log(message):
    ...
    user_procs = active_processes.get(user_id, {})
    if service_id not in user_procs:
        bot.reply_to(message, '❌ Service not found or not running')
        return
    ...
    if not any(t.name == f"logstream-{service_id}" for t in threading.enumerate()):
        thread = threading.Thread(target=stream_logs, args=(user_id, service_id), daemon=True)
        thread.name = f"logstream-{service_id}"
        thread.start()
